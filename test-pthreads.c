...
int main(int argc, char **argv) {
		...
		cinfo.image_width = x1;
		cinfo.image_height = y1;
		...
		JSAMPLE rgb[x1 * tres.num_comps];
		unsigned i;
		for(y = 0; y < y1; y++) {
			for(x = 0; x < x1; x++) {
				for(i = 0; i < tres.num_comps; i++) {
					rgb[x*tres.num_comps+i] = tres.scanlines[y].rgb[x*tres.num_comps+i];
				}
			}
